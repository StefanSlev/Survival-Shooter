...
Game::Game() : Object()
{
	...
	menuLeaderboard->Deactivate();
	menuPages.push_back(menuLeaderboard);
...
void Game::ResetEnemyTargets()
{
	for (Object* gameObject : enemies)
	{
		if (gameObject->IsTypeOf(ObjectType::Enemy))
		{
...
void Game::HandleInGameUpdate()
{
	...
	// check for player entering leaderboard
	if (gameInput.CheckLeaderboardToggle())
	{
		if (!menuLeaderboard->IsAlive())
		{
			menuLeaderboard->Start();
		}
		else 
		{
			menuLeaderboard->End();
		...
		for (size_t indx = 0; indx < nrEnemies; indx++)
		{
			...
			if (enemyTimers[indx] > enemySpawnIntervals[indx])
			{
				...
				if (enemies.size() < MaxNrEnemies && m_gameObjectsHandler)
				{
					...
					auto enemyInitData = std::make_shared<network::EnemyInitData>();
					enemyInitData->m_position = position;
					enemyInitData->m_life = -1;

					m_gameObjectsHandler->AddGameObject(enemyTypes[indx], enemyInitData);
				}
			}
		}
	}
