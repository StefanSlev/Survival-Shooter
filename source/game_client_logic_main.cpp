...
INT WINAPI wWinMain(HINSTANCE hInst, HINSTANCE, LPWSTR, INT)
{
		...
		hermes::communication::Initialize())
	{
		...
		bool hasClientAddress = shared::gameplay::Engine::CheckStartupParameter("client_ip", clientIp);
		...
		if (hasClientAddress && hasServerAddress && hasServerPort && hasWebServerAddress)
		{
			GetLeaderboardService().Initialize(webServerAddress);
			GetOnlineRegistry().Register(&GetLeaderboardService());

			// create game client
			yojimbo::Address clientAddress{ clientIp.c_str() };
